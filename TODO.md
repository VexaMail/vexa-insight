...
- [ ] Scope `ipAddresses.emailsSentCount` to the caller's allowed domains, or stop showing it to restricted users. The 2026-07-26 pass scoped every aggregate the IP views compute from `normalized_events`, but this column is a denormalized lifetime counter maintained at ingest, so a restricted user still sees the IP's global send total next to their own scoped numbers. Fixing it properly means either dropping the column from the restricted response or deriving it per-domain, which is a rollup-shaped change rather than a filter.
- [ ] Reject an empty `allowedDomains` array at the API boundary. `validators/users/{create,update}UserInputSchema.ts` accept `z.array(z.string())` with no `.min(1)`, so `PUT /api/v1/users/:id` can store `"[]"`; only the modal blocks it client-side. `getAllowedDomainIds` now reads that as deny-all (2026-07-26) rather than allow-all, so it is no longer a fail-open, but an admin can still create a user who can see nothing with no feedback.
