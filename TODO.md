...
- [-] Stop re-fetching envelopes for folders that never carry DMARC mail.
      ...
      the nova mailbox: a `.Logs` folder holds 5,038 messages of which 5,037
      fall inside the 30-day window, so every hourly run pulls ~5,000 envelopes
      to discard all of them, while the run's real work is 1-22 reports. The
      30-day window added on 2026-08-25 does not help — the noise folder is
      entirely recent, so bounding by date bounds nothing. The owner is moving
      that log mail out to a separate processor, which removes the cost at
      source and is cheaper than a folder allow-list plus its migration and UI.
      Reopen only if a mailbox shows the same cost with no way to drain the
      noisy folder.
