...
shipping with no check at all. Session-cookie authentication also made
mutating endpoints CSRF-prone.
...
- Wrap every `/api/v1/**` handler in `withApiAuth`
  ...
  1. `requireSameOrigin` (`services/security/requireSameOrigin.ts`) rejects
     unsafe-method requests whose `Sec-Fetch-Site` / `Origin` headers indicate
     a cross-origin caller. Requests authenticated with `x-api-key` or a
     `Bearer` token are exempt: CSRF only matters for cookie-bearing sessions.
  2. `requireAdminAccess` (`services/api/requireAdminAccess.ts`) accepts
...
- Next.js middleware-based auth for `/api`: rejected, the proxy matcher
  excludes `/api` and middleware cannot easily share the timing-safe token
  comparison and session lookup used by the services layer.
