      ...
      {imapAccounts.map((acc, index) => (
        ...
        >
          <div className="flex items-center justify-between gap-2">
            <label
              htmlFor={`install-imap-label-${index}`}
            ...
            {imapAccounts.length > 1 && (
              <button
                type="button"
                onClick={() => dispatch({ type: 'REMOVE_ACCOUNT', index })}
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <label
                htmlFor={`install-imap-server-${index}`}
                className="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300"
              >
                Server
              </label>
              <input
                id={`install-imap-server-${index}`}
                type="text"
                value={acc.server}
                onChange={(e) =>
                  dispatch({
            ...
            <div>
              <label
                htmlFor={`install-imap-port-${index}`}
                className="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300"
              >
                Port
              </label>
              <input
                id={`install-imap-port-${index}`}
                type="number"
                min={1}
                max={65535}
                value={acc.port}
          ...
          <div>
            <label
              htmlFor={`install-imap-username-${index}`}
              className="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300"
            >
              Username
            </label>
            <input
              id={`install-imap-username-${index}`}
              type="text"
              value={acc.username}
              onChange={(e) =>
                dispatch({
          ...
          <div>
            <label
              htmlFor={`install-imap-password-${index}`}
              className="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300"
            >
              Password
            </label>
            <input
              id={`install-imap-password-${index}`}
              type="password"
              value={acc.password}
              onChange={(e) =>
                dispatch({
